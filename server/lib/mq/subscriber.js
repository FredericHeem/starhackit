...
function Subscriber(app, options){
  ...
  Subscriber.prototype.start = function(){
    ...
    .then(function() {
      ...
      return _channel.assertQueue(options.queueName, {exclusive: false});
    })    
    .then(function(res) {
      log.info("bind queue %s, key: %s", res.queue, options.key);
      _queue = res.queue;
      return _channel.bindQueue(_queue, options.exchange, options.key);
    })
    .then(function() {
      //TODO ack
      _channel.prefetch(1);
  ...
  function onIncomingMessage(message){
    log.info("onIncomingMessage ", message.fields);

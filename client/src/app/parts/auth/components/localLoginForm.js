  ...
  function LoginForm({store, login}){
    ...
    return (
      <form className="text-center" onSubmit={e => e.preventDefault()}>
        ...
        <div>
          <LaddaButton className='btn btn-lg btn-primary' data-size={L} data-style={SLIDE_UP} data-spinner-size={30} loading={login.loading} onClick={() => store.login()}>{tr.t('Login')}</LaddaButton>
        </div>
      </form>
    );
  }
